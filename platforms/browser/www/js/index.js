...
var app = {
    ...
    onDeviceReady: function() {
        ...
        app.loadTemplates();
        app.render('container','allPosts',{posts: app.posts});
        app.registerCallbacks();
    },

    loadTemplates: function(){
      var templates = ['allPosts', 'addEntryForm', 'singlePost'];
    ...
    route:function(path){
      ...
      if(/\/posts\/(\d*)/.test(path)){
        var id = parseInt( path.match(/\/posts\/(\d*)/)[1]);
        app.render('container', 'singlePost', {post:app.posts[id]});
        return;
      }
        app.render('container','allPosts',{posts: app.posts});
        return;
